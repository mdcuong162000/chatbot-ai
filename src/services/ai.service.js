...
async function getChatResponse(message, conversationId, options = {}) {
  ...
  const products = knowledgeService.getAllActiveProducts(marketCode);
  const faqs = knowledgeService.getAllActiveFaqs(marketCode, industry);
  ...
  // Tìm FAQ liên quan bằng từ khóa đơn giản (Demo)
  ...
  if (matchedProduct) {
    ...
    - Giá: ${matchedProduct.price}
    - USP (Điểm bán hàng): ${matchedProduct.selling_points ? matchedProduct.selling_points.join(', ') : ''}
    - Tip chốt sale: ${matchedProduct.style_tip}`;
  } else if (matchedFaq) {
    knowledgeContext = `\n\n[TRẠNG THÁI: KHÁCH HỎI KIẾN THỨC CHUNG]
    - Câu hỏi: ${matchedFaq.question}
    - Trả lời: ${matchedFaq.answer}`;
  } else {
    knowledgeContext = `\n\n[TRẠNG THÁI: KHÁCH HỎI CHUNG CHUNG]
    - Thị trường: ${marketCode}
    - Cửa hàng hiện có sản phẩm: ${shopInventory || 'Chưa có sản phẩm nào'}.
    - Nếu khách hỏi thông tin không có trong danh sách, hãy khéo léo mời chào xem các sản phẩm hiện có.
    - TUYỆT ĐỐI KHÔNG BỊA RA thông tin không có trong kịch bản.`;
  }

  // Ưu tiên dùng Groq
  try {
    const aiResult = await getGroqResponse(message, history, knowledgeContext, customerMeta, options, conversationId);
    ...
    const aiResult = await getOpenAIResponse(message, history, knowledgeContext, customerMeta, options);
...
async function getGroqResponse(message, history = [], knowledgeContext = '', customerMeta = null, options = {}, conversationId = null) {
  try {
    const settings = await getSettings();
...
Ngắn gọn (max 3 câu), tập trung khám phá nhu cầu. Kiến thức hỗ trợ: ${knowledgeContext}`;
      }
    }
...
async function getOpenAIResponse(message, history = [], knowledgeContext = '', customerMeta = null, options = {}) {
  try {
    ...
    const systemPrompt = isReturning 
      ? `Bạn là nhân viên Sales thân thiết. Chăm sóc khách cũ đã mua: ${customerMeta.purchased_products}. Sản phẩm hiện tại: ${knowledgeContext}`
      : `Bạn là nhân viên Sales mới. Chào khách và tư vấn sản phẩm: ${knowledgeContext}`;
